...
export function updateTableColumns(selectedValue) {
  ...
  if (needsTimeQuantum && !hasTimeQuantumColumn) {
    ...
    tqHeader.innerHTML = `<div class="title-yellow flex-fill text-center">TQ</div>`;
...
function validateTableInputs(algorithm, options = {}, mode) {
  ...
  document.querySelectorAll(".loading-s").forEach((b) => { b.classList.add("hides") });
  document.querySelectorAll(".loading-l").forEach((b) => { b.classList.remove("hides") });
  resetUI();
  setTimeout(() => {
    document.querySelectorAll(".loading-s").forEach((b) => { b.classList.remove("hides") });

    document.querySelectorAll(".loading-l").forEach((b) => { b.classList.add("hides") });
...
function resetUI() {
  ...
  const resultTables = ["completion", "process", "cpuTotal", "cpuUtil", "timeline"];

  resultTables.forEach(id => {
    const el = document.getElementById(id);
    if (el) el.innerHTML = "";
  });
  ...
  tb.innerHTML = "";
  const tbl = document.querySelector("#resultTableWaitingTime tbody");
  tbl.innerHTML = "";
}
...
document.getElementById("calculate").addEventListener("click", function () {
  ...
  if (horizontalLine) {
    horizontalLine.style.display = "block"; // Show the horizontal line
  }
...
function restartApp() {
  ...
  // Reset algorithm selection to default (e.g., FCFS)
  const defaultRadio = document.getElementById("btnradio1");
  if (defaultRadio) {
  ...
  // Clear all rendered UI elements
  if (typeof resetUI === "function") resetUI();

  // Additionally, clear any custom result tables or timeline content outside resetUI if needed
  const resultTables = ["completion", "process", "cpuTotal", "cpuUtil", "timeline"];
  ...
  // Hide horizontal line after restart
  const horizontalLine = document.querySelector(".horizontal-line");
  if (horizontalLine) horizontalLine.style.display = "none";

  // Ensure at least one row exists after restart
  if (typeof addRow === "function") addRow("#processTable", "FCFS");
}
